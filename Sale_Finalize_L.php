...
if ($select_query && mysqli_num_rows($select_query) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($select_query)) {
        ...
        $update = "UPDATE product_latkar SET Quantity = Quantity - $buy_quantity WHERE Product_ID = '$product_id'";
    ...
    echo "<script>window.location='Sale_latkar.php';</script>";
    exit();
} else {
    echo "<script>alert('No pending sale found.'); window.location='Sale_latkar.php';</script>";
}
?>
